...
This report documents task 2 testing and evaluation of the Gomoku game. The program was first developed in task 1 as a terminal-based game. In this task 2, different tests will be conducted to see whether the program run as expected, and also collect feedback from different users. Then, we will fix bugs in this task if found, and new features may be added to improve game experience based on feedback and testing.
...
Unit tests were performed on each subprogram (function) in the code. A complex test can be split into different small task by creating subprogram. Each subprogram have specific actions, they can be called repeatedly without rewriting the code. 
...
| UT-4 | `check_win_v2` | Diagonal win (Bottom-left > Top-right) | `Cell (8,4),(7,5),(6,6),(5,7),(4,8)` = `"●"` -> Execute `check_win_v2(6, 6, "●")` | True | Passed |
...
| UT-7 | `is_board_full` | 1 cell occupied | 1 cell occupied, remaining empty | False | Passed |
...
| --- | --- | --- | --- | --- |
| 1 | Launch the game, then select mode `"1"` | No error message, PvP mode is entered | PvP mode | Pass |
...
| --- | --- | --- | --- | --- |
| 1 | Launch the game, then select mode `"1"` | No error message, PvP mode is entered | PvP mode | Pass |
| 2 | Player 1: `(3,7),(4,7),(5,7),(6,7),(7,7)`; Player 2 randomly place at other position | Piece placed; Two players input coordinate one by one. | As expected | Pass |
...
| 3 | After the last piece filled the board | `is_board_full` should return board full, and the game have no winner | "Board Full. No winner.", draw detected, round is end | Pass |
...
| Step | Test Step / Input | Expected | Actual | Pass/Fail |
| --- | --- | --- | --- | --- |
| 1 | Launch the game, then select mode `"2"` | No error message, PvM mode is entered | PvM mode | Pass |
...
| 5 | Enter same coordinate twice time | `"This cell is already occupied"` should show at the second time | Message shown | Pass |
...
![feat_play-again_score-tracking](https://filebox.aye.moe/share/select?code=B85Q5)
