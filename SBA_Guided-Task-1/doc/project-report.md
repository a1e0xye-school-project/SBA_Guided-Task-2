...
<div align="center">
<img src="https://a1e0xye-sch-prj-ict-sba-t1-sync.pages.dev/doc/img/prog-logic-flow_v2.jpeg" style="zoom:50%">
</div>
...
| --- | --- | --- | --- |
| `turn` | variable | 0 [Ln 16]| Recorded which player's turn currently. The value will be 0 or 1, as the represent of player 1 or 2 |
| `empty_cell_indicator` | constant | (Custom value) [Ln 17]| When the cell on the board is empty, the board will show this value as it indicate a empty cell. |
| `player_1_indicator` | constant | (Custom value) [Ln 18] | The indicator for player 1. It will show on the board cell if the player selected the cell. It should be different from player 2. |
| `player_2_indicator` | constant | (Custom value) [Ln 19] | The indicator for player 2. It will show on the board cell if the player selected the cell. It should be different from player 1. 
| `board` | 2D List | Filled with `empty_cell_indicator` [Ln 21-26] | Stored all cell value.
| `num_row_column` | constant | (Custom value) [Ln 22] | The value determine the size of the board (if num_row_column = 15, then the board will have 15 rows and columns). The board size can be easily changed by change the value of this varibale.
...
| Function Name | Used Line |
| --- | --- |
| cprint | Ln 40,41,111,112,114,115,122,172,184,193,204,206,213,219,226 |
...
| `game_mode` | Global | Stored user selected game mode (`PvP` or `PvM`) & control which logic should run. |
...
| `Main loop (PvM block)` | `last_player_row_index`, `last_player_column_index`, `nearby_empty_cells`, `temp_row`, `temp_column`, `current_check_row`, `current_check_column`, `pvm_machine_selected_move`, `empty_row`, `empty_column`, `all_empty_on_board`, `row_index`, `column_index` | Used temporarily for machine move decision flow (search nearby cells, defense/attack checks, and fallback random selection). |
...
| Stages | Notes |
| --- | --- |
| Place move | [Ln 199, 201] Write `player_1_indicator` or `player_2_indicator` into the chosen cell in `board`. |
| Win check | [Ln 210, 216] Calls `check_win_v2(board, row_index, col_index, indicator)` to check if the move created five or more in a line in any of four directions (horizontal, vertical, two diagonals) |
| Draw check | [Ln 223] If in stage "Win check" returned the player have'nt win yet, then call `is_board_full(board)` to check if any empty cell remains in the game board. If no remaining empty cell, the game reaches a stalemate. |
...
## Player vs Machine (PvM) mode

The project supports two game mode: 
...
Before the gomoku game starts, the program will ask user to choose their prefered game mode in terminal by tying in number 1 or 2 (If data validation not passed which mean user did't enter correct number, the program will keep asking to retry.) After selecting the game mode, the program will display the selected game mode 1.5 seconds(By applying time.sleep())

When `PvM` mode is selected, Player 1 will be human(user) and Player 2 will be the machine. On machine turn (`turn == 1`), the program will runs a simple decision logic flow to select move automatically without any help.

When `PvP` mode is selected, Player 1 and 2 will all be human player. The program will prompt which player turns currently, and required player to enter their move. No machine decision involved in PvP mode.

### PvM machine decision logic flow

| Step | Logic | Notes |
| --- | --- | --- |
| 1 | Search nearby empty cells | Build a list of empty cells in a 3x3 area around the human player's latest selected move. |
| 2 | Defense | Check nearby empty cell can immediately block the humman player winning. |
| 3 | Attack | If no defense is needed, the program will check whether any move can extend its own line by selecting a nearby position. |
| 4 | Fallback: Random move | If no defensive/attacking move is selected in previous logic step, the program will randommly choose an empty nearby empty cell. If there is no empty cell nearby, the program will then choose any one empty cell in whole board. |
| 5 | Apply move | Place machine piece to the game board , and display machine selected move to human player. |
...
if turn == 1 and game_mode == "PvM":        # PvM mode
    ...
    if not pvm_machine_selected_move:
        if len(nearby_empty_cells) > 0:
            pvm_machine_selected_move = random.choice(nearby_empty_cells)
        else:
            all_empty_on_board = []
            for row_index in range(num_row_column):
            ...
            pvm_machine_selected_move = random.choice(all_empty_on_board)
...
To enhance game experience, making the machine like human "thinking", the program used `time.sleep()` before applying machine selected move to leave a thinking time. And also a time for human player to check machine selected move after printed machine selected move. The pause time can be changed by replacing the value inside the `time.sleep()`, the unit for this pause time is second(s).

This PvM logic is a **rule-based strategy** and does not call external API. It is simple and allow run locally. Thanks to the modular design in this program, we can use `check_win_v2()` to check which cell can defense or attack without rewriting. The machine decision logic flow is not the strongest as its goal is not to win but to defense human player, but it did provide a playable single-player experience.
...
The project included both PvP and PvM mode. The program used a simple dicision logic to select a move if PvM mode selected, making this game more playable. The interface still rely on terminal, so color rendering(termcolor) and screen clearing (ANSI) may vary across different environments/platforms. 

Future project refactoring could focus in followed area:

1. Improve Machine decision logic: Use a new logic to improve machine move quality and reduce random behavior in the game. The machine can aims to win this game but not defensing human player. AI dicision can be achieved by calling external API, however, the availability of the API is still a problem.

2. Graphical User Interface (GUI): Render the game board as a clickable grid. More elements like music, animations, and timer can be added into GUI with a standalone window show all content together. Similar GUI design can achieve by using external library like `pygame` or show the window in web browser by using web application framework like `Flask`.

---

## Program full code

![full-code-gh-long-sc](https://a1e0xye-sch-prj-ict-sba-t1-sync.pages.dev/doc/img/gh-code-long.png)
...
Github Repo: https://github.com/a1e0xye-school-project/SBA_Guided-Task-1
